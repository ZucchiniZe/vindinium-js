...
Bot.prototype.sendRequest = function (){
  ...
  return new Promise(function(resolve, reject){
    ...
    if(_this.gameStart === true){
      ...
      request.post({url: _this.URL, form: {key: _this.key, map: "m1", turns: 50}}, function(err, res, body) {
 ...
 return new Promise(function(resolve, reject){
  ...
  for(var i = 0; i < (tempMapSize * tempMapSize * 2); i+=2){
    ...
    table = new Table({chars: {'top':'','top-mid':'','top-left':'',
      ...
      head: ['', 'Life', 'Mines', 'Gold', 'Crashed', 'ELO']
    })
    console.log('\033[2J');

    var totals = {'Total':[0, 0, 0]}
    for(i=0;i<4;i++) {
      var botname = bot.data.game.heroes[i].name
      if(bot.data.game.heroes[i].name == 'random') botname = 'Random ' + i
      var name = _this.data.hero.id-1 === i ? 'My Bot' : botname
      var obj = {}
      totals['Total'][0] += bot.data.game.heroes[i].life
      totals['Total'][1] += bot.data.game.heroes[i].mineCount
      totals['Total'][2] += bot.data.game.heroes[i].gold
      if(typeof bot.data.game.heroes[i].elo == 'undefined') bot.data.game.heroes[i].elo = 0
      obj[name] = [bot.data.game.heroes[i].life, bot.data.game.heroes[i].mineCount, bot.data.game.heroes[i].gold, bot.data.game.heroes[i].crashed, bot.data.game.heroes[i].elo]
      table.push(obj)
    }
    table.push(totals)
...
Bot.prototype.getNeighbors = function(pos) {
  var current = [this.yourBot.pos.x, this.yourBot.pos.y];
  var neighbors = [];

  var grid = new PF.Grid(this.gridLength, this.gridLength, this.useThisMap)
  grid.getNeighbors(grid.getNodeAt(current[0], current[1]), 1).forEach(function(node) {
    neighbors.push([node.x, node.y])
  })
}
